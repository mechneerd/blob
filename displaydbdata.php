...
if($result->num_rows>0){
	...
	while($rows=$result->fetch_assoc()){
		...
		'<td style="border:1px solid">'.'<a href="download.php?id='.$rows["emp_id"].' " download>'.'<img src="data:image/jpeg;base64,'.base64_encode($rows['image']).'"  width="50px" height="50px" />'.'</a>'.'</td>'.'<td>'.'<a text-decoration: "none"; href="updatedb.php?id='.$rows["emp_id"].' ">'.'<button style="background-color: #4CAF50;">'.'Update'.'</button>'.'</a><a style="margin-left:20px;" text-decoration: "none"; href="deletingselectedrows.php?id='.$rows["emp_id"].'">'.'<button style="background-color: #f44336;">'.'Delete'.'</button>'.'</a>'.
