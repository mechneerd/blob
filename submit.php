...
$stmt = $con->prepare("INSERT INTO formdata (user_name,password,emp_number,website,gender,movie1,movie2,movie3,image,type,name,size) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
$stmt ->bind_param("siisssssssss", $name,$pass, $emp ,$web ,$gen ,$mov1,$mov2,$mov3,$tmp,$filetype,$filename,$file_size);

$stmt -> execute();
header("Location:displaydbdata.php ");
/*if($stmt -> execute()){
	echo 'image also inserted';
